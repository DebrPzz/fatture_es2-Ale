<?php 
	...
	$data=R::findAll($tbl, 'ORDER by id ASC LIMIT 999');
        ...
        if (!empty($_REQUEST['nome'])) :
                        ...
                        if($nome=="Tutti"){$data=R::find('riceclienti','where dataemissione between "'.$fromfil.'"and"'.$tofil.'"');};
                        $data=R::find('riceclienti','where nome = "'.$nome.'"'.'or (dataemissione between "'.$fromfil.'"and"'.$tofil.'")');
		...
		<div class="col-sm-12">
                    <form method="post" action="index.php?p=ricevute">
                        <div class="form-group">
                            ...
                            <div class="col-sm-2">
                            <input class="form-control" name="tofil" value="<?=$dot->tofil?>" type="date" min="<?=$dot->fromfil?>" max="<?=date("Y-m-d")?>"/>
                            </div>
